...
from taskiq import AsyncBroker, InMemoryBroker, PrometheusMiddleware
...
if app_config.ENVIRONMENT == "testing" and not is_worker_process:
    ...
    broker = ListQueueBroker(url=app_config.QUEUE_REDIS_BROKER_URL).with_middlewares(
        PrometheusMiddleware(server_port=9002)
    )
