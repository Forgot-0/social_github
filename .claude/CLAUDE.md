...
## Codebase Intelligence for social_github (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-08-07 (commit 670e37c). Confidence: 94%.
...
### Architecture
**Files:** 646 | **Lines:** 38784 | **Import cycles:** 6
social_github is a python codebase of 646 files. Execution starts at app/core/configs/app.py, app/main.py, frontend-chat-client/index.html and 1 other entry point. Start here when reading the codebase. Ranked by PageRank over the import graph: the files most of the codebase ultimately depends on.

### Key modules
- `app/core` — app/core · app/core/api · app/core/configs · app/core/consumers · app/core/db · app/core/di · and 18 more
**Language:** python | **Files:**…
- `app/core/events` — app/core/events · app/core/events/mediator · app/core/filters
**Language:** python | **Files:** 12 | **Public symbols:** 57 / 57
Covers the…
- `app/core/db` — app/core/db
**Language:** python | **Files:** 6 | **Public symbols:** 44 / 48
Covers the 6 source files in app/core/db
- `app/profiles` — app/profiles · app/profiles/commands · app/profiles/commands/profiles · app/profiles/consumers · app/profiles/dtos · app/profiles/events ·…
- `app/core/services` — app/core/services · app/core/services/auth · app/core/services/cache · app/core/services/cache/aiocache
**Language:** python | **Files:**…
- `app/auth/models` — app/auth/models
**Language:** python | **Files:** 7 | **Public symbols:** 42 / 42
Covers the 7 source files in app/auth/models
- `app/core/configs` — app/core/configs · app/core/consumers
**Language:** python | **Files:** 6 | **Public symbols:** 14 / 15
Covers the 6 source files in 2…
- `app/chats/models` — app/chats/models
**Language:** python | **Files:** 10 | **Public symbols:** 86 / 88
Covers the 10 source files in app/chats/models
- `app/projects/models` — app/projects/models
**Language:** python | **Files:** 7 | **Public symbols:** 37 / 39
Covers the 7 source files in app/projects/models
- `app/chats` — app/chats · app/chats/commands · app/chats/commands/attachments · app/chats/commands/calls · app/chats/commands/chats ·…

### Entry points
- `app/main.py`
- `frontend-chat-client/main.js`
- `app/core/configs/app.py`

### Files that need care (bug-fix history first, then churn — check `get_risk` before editing)
- `app/chats/models/message.py` — 7 bug fixes, last fix today (bug magnet); 9 commits/90d
- `app/main.py` — 5 bug fixes, last fix yesterday (bug magnet); 9 commits/90d
- `app/chats/config.py` — 4 bug fixes, last fix 3 days ago; 7 commits/90d
- `app/chats/commands/chats/add_member.py` — 5 bug fixes, last fix 3 days ago; 3 commits/90d
- `app/chats/commands/chats/ban_member.py` — 5 bug fixes, last fix 3 days ago; 4 commits/90d

### Code health
Three co-equal signals: defect risk 7.97/10 avg, hotspot health 5.69/10 (stable), worst `app/chats/routes/v1/ws.py` at 1.0/10 · maintainability 9.1/10 · performance risk 29 open static I/O-in-loop / N+1 findings. Detail: `get_health()`.

Critical files:
- `app/auth/exceptions.py` — untested hotspot — impact −2.0
- `app/auth/models/user.py` — untested hotspot — impact −2.0
- `app/auth/services/jwt.py` — untested hotspot — impact −2.0
- `app/chats/config.py` — untested hotspot — impact −2.0
- `app/chats/exceptions.py` — untested hotspot — impact −2.0
