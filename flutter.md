...
4. Datasource — точные пути и нюансы из api-docs.md:
   ...
   - PATCH /chats/{chat_id}/members/{user_id}/ban/ — тело {reason?, bannet_to?} — да, поле называется bannet_to (опечатка в реальном API), назови Dart-параметр нормально (bannedTo), но при сериализации в JSON ключ должен быть именно "bannet_to".
